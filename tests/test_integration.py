...
class IntegrationTests(unittest.TestCase):
    ...
    def test_processing_feedback_report_and_provenance(self):
        ...
        self.assertEqual(status['results_count'],1)
        self.assertEqual(status['search_query']['entities'][0]['entity_type'],'person')
        ...
        results=self.client.get(self.base+'/results').json()
        self.assertEqual(len(results),1)
        self.assertEqual(results[0]['result_id'],'legacy-track:1')
        self.assertEqual(results[0]['entities'][0]['label'],'person')
        self.assertEqual(results[0]['model_provenance'][0]['provider'],'legacy-person-pipeline')
